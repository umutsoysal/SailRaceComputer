...
class _SimulatorScreenState extends State<SimulatorScreen> {
  ...
  void _onCourseChanged(Course updated) {
    ...
    if (_currentMark >= updated.buoys.length) {
      setState(
        () => _currentMark = (updated.buoys.length - 1).clamp(
          0,
          updated.buoys.length,
        ),
      );
    }
  }
  ...
  Future<Course> _loadDemoCourse() async {
    try {
      final raw = await rootBundle.loadString(
        'assets/courses/demo_chicago.srcourse.json',
      );
  ...
  Widget build(BuildContext context) {
    ...
    final dist = mark == null
        ? null
        : distanceMeters(_sim.position, mark.position);
    final brg = mark == null
        ? null
        : bearingDegrees(_sim.position, mark.position);
            ...
            onPressed: () {
              setState(() {
                ...
                if (course.buoys.isNotEmpty) {
                  _sim.setHeading(
                    bearingDegrees(_sim.position, course.buoys.first.position),
                  );
                }
              });
            },
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Expanded(
                child: _metric(
                  context,
                  'SOG',
                  sim.speedKnots.toStringAsFixed(2),
                  'kn',
                ),
              ),
              const SizedBox(width: 8),
          ...
          OutlinedButton.icon(
            ...
            label: Text(
              mark == null ? 'Aim at mark' : 'Point heading at ${mark!.name}',
            ),
            onPressed: mark == null
                ? null
                : () => sim.setHeading(
                    bearingDegrees(sim.position, mark!.position),
                  ),
          ),
          const SizedBox(height: 12),
          Text(
